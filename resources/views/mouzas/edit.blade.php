        ...
        <div class="col-lg">
            <div class="card mb-4">
                <div class="card-header d-flex align-items-center justify-content-between">
                    <h5 class="mb-0">{{ __('থানা আপডেট') }}</h5>

                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('mouzas.update', $mouza->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="row mb-3">
                            <label class="col-sm-2 col-form-label" for="name">{{ __('থানার নাম') }}</label>
                            <div class="col-sm-10">
                                <input type="text" class="form-control @error('name') is-invalid @enderror"
                                    id="name" name="name" value="{{ old('name', $mouza->name) }}"
                                ...
                                <select name="location_id" id="location_id" class="form-select select2">
                                    ...
                                    @foreach ($locations as $location)
                                        <option value="{{ $location->id }}"
                                            {{ $mouza->location_id == $location->id ? 'selected' : '' }}>
                            ...
                            <div class="col-sm-10">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="status" checked=""
                                        name="status" value="1" {{ $mouza->status == 1 ? 'checked' : '' }}>
